...
//Valores que vão servir mesmo quando
//adicionarmos mais restaurantes e pratos
int numRestaurantes = NUM_RESTAURANTES;
int numPratos = NUM_RESTAURANTES*NUM_PRATOS_POR_RESTAURANTE;
...
int findPratoIndexByCode(int codePrato);
int findPratoIndexByRestAndCode(Restaurante restaurante, int codePrato);
...
void mostrarPratos(Restaurante restaurante) {
    ...
    for (int j = 0; j < NUM_PRATOS_POR_RESTAURANTE; j++) {
        ...
        if(restauranteIndex != -1) {
            Prato prato = pratos[restauranteIndex * NUM_PRATOS_POR_RESTAURANTE + j];
...
int main() {
    ...
    restaurantes = (Restaurante*)malloc(NUM_RESTAURANTES * sizeof(Restaurante));
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (opcao == 0) {
                ...
                int j = findPratoIndexByRestAndCode(restaurantes[i], pratoEscolhido);
                if (j != -1) {
...
int findPratoIndexByCode(int codePrato) {
    for(int i = 0; i < numPratos; i++) {
        if(codePrato == pratos[i].codigoPrato)
            return i;
    }
    return -1;
}
...
int findPratoIndexByRestAndCode(Restaurante restaurante, int codePrato) {
    int pratoIndex = findPratoIndexByCode(codePrato);
    if(pratos[pratoIndex].codigoRest != restaurante.codigo)
        return -1;
